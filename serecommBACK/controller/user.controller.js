...
exports.signup = async (req, res,next) => {
  ...
  try {
    ...
    console.log ("Estoy en user.controller - line 14 - email: "+ req.body.email)
    if (user) {
      console.log ("Estoy en user.controller - line 17 - email: "+ req.body.email)
      res.send({ status: "failed", message: "Email already exists" });
    } else {
      
      const saved_user = await User.create(req.body);
      
      // generateConfirmationToken comes from User ../model/User
      const token = saved_user.generateConfirmationToken();

      await saved_user.save({ validateBeforeSave: false });

      console.log("Estoy en user.controler - line 28 - signup - Successfully Registered ....: "+ saved_user)
      res.status(200).json({
      status: "success",
...
module.exports.login = async (req, res,next) => {
  try {
    ...
    // comparePassword comes from User ../model/User
